...
.controller('UserMessagesCtrl', ['$scope', '$rootScope', '$state', '$stateParams', 'myFactory', '$ionicActionSheet', '$ionicPopup', '$ionicScrollDelegate', '$timeout', '$interval', '$cordovaSQLite', '$ionicHistory', 'toServer',  function($scope, $rootScope, $state, $stateParams, myFactory, $ionicActionSheet, $ionicPopup, $ionicScrollDelegate, $timeout, $interval, $cordovaSQLite, $ionicHistory, toServer) {

    //----------------------------------------------------------------
  
        //Add user called nickname
        
        toServer.on('new message', function(data) {
            console.log(data);
        });
     ...
     */
        ...
        messageCheckTimer = $interval(function() {// here you could check for new messages if your app doesn't use push notifications or user disabled them
        }, 20000);
    });
    ...
    function getMessages() {
        var query = "select * from UserMessages where userId = '" + $scope.toUser._id + "'";
        //'sandeep123'"; 
    ...
    $scope.sendMessage = function(sendMessageForm) {
        ...
        // toServer.emit('message', message);
        toServer.emit('new message', message)
        ...
        $timeout(function() {
            ...
            $scope.hardcodeValues(repliedMessage);
            //to save automatic hard coded reply messages
 ...
 */
    ...
    $scope.viewProfile = function(msg) {
        if (msg.userId === $scope.user._id) {// go to your profile
        } else {// go to other users profile
        }
    }
    ;
