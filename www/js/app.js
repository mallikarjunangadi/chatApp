angular.module('chatApp', ['ionic', 'ngCordova', 'monospaced.elastic', 'angularMoment', 'chatApp.controller', 'chatApp.services', 'chatApp.filter', 'chatApp.directive', 'btford.socket-io','ngSanitize'])

.run(function($ionicPlatform, $cordovaSQLite, $rootScope, $http) {
  $ionicPlatform.ready(function() { 
    ...
    if (window.cordova) {
      $rootScope.db = $cordovaSQLite.openDB({ name: "chatApp.db", location: 'default' }); //device
     console.log("Android");
    }else{
      $rootScope.db = window.openDatabase("chatApp.db", '1', 'mail', 1024 * 1024 * 100); // browser
      console.log("browser");
    } 
...
.config(function($stateProvider, $urlRouterProvider) {
  ...
  .state('tabs', {
   ...
   .state('channelMembers', {
    url: '/channelMembers',
    templateUrl: 'templates/channelMembers.html',
    controller: 'channelMembersCtrl'
  })
